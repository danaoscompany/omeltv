...
class User extends CI_Controller {
	...
	public function find_nearest_partners() {
		...
		if ($category == 'all' || $category == 'both') {
			...
			if (sizeof($partners) > 0) {
				$partners[0]['command'] = "SELECT *, SQRT(POW(69.1 * (latitude - " . $lat . "), 2) + POW(69.1 * (" . $lng . " - longitude) * COS(latitude / 57.3), 2)) AS distance FROM `users` WHERE `id`!=" . $userID . " AND `id` NOT IN " . $skippedIDs . " AND `gender`='" . $category . "' AND `is_searching`=1 AND `country_code`='" . $countryCode . "' AND (`user_find_candidate`=0 OR `candidate_user_id`=" . $userID . ") ORDER BY RAND();";
			}
		}
		/* FOR TEST ONLY */
		if ($userID == 8) {
	...
	public function user_remove_self_candidate() {
		$userID = intval($this->input->post('user_id'));
		$this->db->query("UPDATE `users` SET `user_find_candidate`=0 WHERE `id`=" . $userID);
		$this->db->query("UPDATE `users` SET `candidate_user_id`=0 WHERE `id`=" . $userID);
	}
